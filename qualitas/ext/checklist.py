...
def render_item(caption, checked):
    if checked:
        return '<p><label><input type="checkbox" checked="checked"/><span>{0}</span></label>'.format(caption)
    else:
        return '<p><label><input type="checkbox"/><span>{0}</span></label>'.format(caption)
